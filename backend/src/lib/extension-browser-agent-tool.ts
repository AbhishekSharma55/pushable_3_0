      ...
      new DynamicStructuredTool({
        ...
        func: async (params) => {
          ...
          deductCredits({
            ...
            logger.warn({ err }, 'Extension browser agent: browser action credit deduction failed'),
          );
  ...
  return new DynamicStructuredTool({
    ...
    func: async ({ instruction }) => {
      ...
      if (invocationCount > MAX_INVOCATIONS) {
        ...
        return (
          `STOPPED: Browser agent limit reached (${MAX_INVOCATIONS} calls). ` +
      ...
      try {
        ...
        const resolvedModel = browserModelId || 'openai/gpt-5.4-mini';
        ...
        // --- Agent node ---
        const agentNode = async (state: typeof ExtBrowserAgentState.State) => {
          ...
          deductCredits({
            ...
            logger.warn({ err }, 'Extension browser agent: LLM credit deduction failed'),
          );
          ...
          if (aiResponse.tool_calls && aiResponse.tool_calls.length > 0) {
            ...
            const textOut =
              typeof aiResponse.content === 'string'
                ? aiResponse.content.slice(0, 300)
                : JSON.stringify(aiResponse.content).slice(0, 300);
        ...
        const toolNode = async (state: typeof ExtBrowserAgentState.State) => {
            ...
            logger.info(
              {
                ...
                result: resultContent.slice(0, 800),
              },
              `📦 TOOL RESULT [${tc.name}]`,
            );
        ...
        // --- Router ---
        const shouldContinue = (state: typeof ExtBrowserAgentState.State) => {
