...
if (credential) {
  console.log(
    `Auth: ${credential.isApiKey ? 'API key' : 'OAuth token'} loaded (${credential.token.slice(0, 12)}…)`,
  );
...
function forwardToAnthropic(bodyStr, res, isStreaming) {
  return new Promise((resolve) => {
    ...
    // OAuth tokens (from Claude Code subscription) need Bearer auth;
...
async function handleMessages(req, res) {
  ...
  console.log(
    `[proxy] ${new Date().toISOString()} model=${model} msgs=${messages?.length} tools=${tools?.length || 0} stream=${!!stream} auth=${credential?.isApiKey ? 'api-key' : 'oauth'}`,
  );
  ...
  if (directResult.statusCode === 200 || directResult.statusCode === 201) {
    console.log(`[proxy] Direct API success (tools in request: ${tools?.length || 0})`);
  ...
  console.log(
    `[proxy] Direct API failed (${directResult.statusCode}), body preview: ${directResult.body?.slice(0, 200)}`,
  );
  if (tools?.length) {
    console.warn(
      `[proxy] WARNING: CLI fallback does NOT support native tool calling. ${tools.length} tools will be injected as text in system prompt (unreliable).`,
    );
  }
  console.log(`[proxy] Falling back to claude --print`);
