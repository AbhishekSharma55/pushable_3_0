...
It is a real, running product. The codebase is **~55 PostgreSQL tables, 36 REST route modules, a ~2,500-line LangGraph agent state machine, 10 Docker services, and 3 Next.js frontends**.
...
The platform runs as 10 Docker services (a single shared deployment — **not** one container per user; tenant isolation is enforced in software, with every business table scoped by `workspaceId`):
...
| **browser-service**  | Python + FastAPI + Camoufox/Playwright | 8080        | Anti-detect headless browser (Firefox-based)     |
| **extension-bridge** | Node.js + WebSocket                    | 3004        | Chrome extension relay (user's real browser)     |
| **cdp-analyzer**     | Python + FastAPI + websockets          | 5050        | Chrome DevTools Protocol DOM analysis            |
...
| **LLM Gateway**  | OpenRouter (Claude, GPT, Gemini — routes to any OpenRouter-hosted model) |
| **Browser**      | Camoufox / anti-detect Firefox (cloud) + Chrome Extension (real browser) |
...
- 20 preconfigured LLM models (GPT-5.x, Claude, Gemini) via OpenRouter — the gateway can route to any OpenRouter-hosted model
...
- Cloud browser (headless Camoufox / anti-detect Firefox with proxy and CAPTCHA support)
...
- [`backend/src/lib/gateway.ts`](./backend/src/lib/gateway.ts) — single abstraction over any OpenRouter-hosted model (Claude, GPT, Gemini, …), with direct Anthropic routing + prompt caching when an API key is set.
- [`browser-service/src/`](./browser-service/src) — Camoufox-driven anti-detect Firefox automation with proxy + CAPTCHA support.
- [`extension-bridge/`](./extension-bridge) + [`extension-v4/`](./extension-v4) — WebSocket relay that lets the backend automate the user's _real_ Chrome browser when bot detection is a problem.
- [`backend/src/db/schema/`](./backend/src/db/schema) — ~55 Drizzle tables; every business table is `workspaceId`-scoped.
...
PostgreSQL 16 with pgvector extension. ~55 tables managed by Drizzle ORM.
...
16. [Database Schema](./docs/platform/16-database-schema.md) (~55 tables)
